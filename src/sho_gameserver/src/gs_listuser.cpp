...
CUserLIST::CUserLIST(UINT uiInitDataCNT, UINT uiIncDataCNT):
    ...
    char szTmp[MAX_PATH];
    t_HASHKEY HashKey;
    m_HashLevelUpTRIGGER = new t_HASHKEY[GameStaticConfig::MAX_LEVEL];
    ::ZeroMemory(m_HashLevelUpTRIGGER, sizeof(t_HASHKEY) * GameStaticConfig::MAX_LEVEL);

    for (int iL = 1; iL < GameStaticConfig::MAX_LEVEL; iL++) {
...
//-------------------------------------------------------------------------------------------------
void
CUserLIST::InitData(classUSER* pData) {
    pData->Init_GameOBJ();
    pData->InitUSER();
}
void
CUserLIST::FreeData(classUSER* pData) {
...
CUserLIST::DeleteUSER(classUSER* pUSER, BYTE btLogOutMODE) {
    ...
    if (pUSER->m_HashACCOUNT) {
        ...
        if (!m_pHashACCOUNT->Delete(pUSER->m_HashACCOUNT, pUSER)) {
            g_LOG.CS_ODS(0xffff,
                "ERROR *** Sub_ACCOUNT( %s : %s) not found \n",
                pUSER->Get_ACCOUNT(),
                pUSER->Get_IP());
        }
...
classUSER*
CUserLIST::Find_CHAR(char* szCharName) {
...
//-------------------------------------------------------------------------------------------------
bool
CUserLIST::Add_ACCOUNT(int iSocketIDX, t_PACKET* pRecvPket, char* szAccount) {
    ...
    if (pUSER->Send_srv_JOIN_SERVER_REPLY(pRecvPket, szAccount)) {
        if (RESULT_CONFIRM_ACCOUNT_OK == pRecvPket->m_wls_CONFIRM_ACCOUNT_REPLY.m_btResult) {
            ...
            {
                short nOffset =
                    sizeof(wls_CONFIRM_ACCOUNT_REPLY) + (short)strlen(szAccount) + 1; // + '\0'
...
//-------------------------------------------------------------------------------------------------
classUSER*
CUserLIST::Find_ACCOUNT(char* szAccount) {
...
//-------------------------------------------------------------------------------------------------
bool
CUserLIST::Kick_ACCOUNT(char* szAccount) {
...
//-------------------------------------------------------------------------------------------------
void
CUserLIST::Send_zws_ACCOUNT_LIST(CClientSOCKET* pSrvSocket, bool bSendToGUMS) {
    classPACKET* pCPacket = Packet_AllocNLock();
