...
private:
    ...
    bool Send_gsv_CREATE_ITEM_REPLY(BYTE btResult,
        short nStepORInvIDX = -1,
        float* pProcPOINT = NULL,
        tagITEM* pOutItem = NULL);
    ...
    bool Send_gsv_SET_TWO_INV_ONLY(WORD wType,
        BYTE btInvIdx1,
        tagITEM* pITEM1,
        BYTE btInvIdx2,
        tagITEM* pITEM2);
    ...
    short
    Recv_cli_REVIVE_REQ(BYTE btReviveTYPE, bool bApplyPenalty = false, bool bSkipPayment = false);
    ...
    void LogCHAT(const char* szMSG,
        ...
        const char* szMsgTYPE); // 채팅을 파일에 로그로 남김 ( 대만일 경우에만 )
    ...
    bool Recv_cli_SET_WEIGHT_RATE(BYTE btWeightRate /*t_PACKET *pPacket*/,
        bool bSend2Around = false);
...
public:
    ...
    bool
    Send_gsv_BILLING_MESSAGE_EXT(WORD wMsgType, DWORD dwPayType, DWORD* pPlayFlag, char* szMsg);
    ...
    int Quest_GetPartyLEVEL() { return this->GetPARTY() ? this->GetPARTY()->GetPartyLEV() : -1; }
...
public:
    ...
    bool Dec_ShotITEM(BYTE btShotCnt) {
        if (this->GetCur_MOVE_MODE() <= MOVE_MODE_RUN && this->m_pShotITEM) {
            ...
            } else if (0 == (m_pShotITEM->GetQuantity() & 0x0f)) {
...
private:
    ...
    short Cheat_where(CStrVAR* pStrVAR,
    ...
    short
    Cheat_npc(CStrVAR* pStrVAR, CObjNPC* pNPC, int iNpcIDX, char* pArg2, char* pArg3, char* pArg4);
