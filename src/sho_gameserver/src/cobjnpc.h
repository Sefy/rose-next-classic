...
public:
    ...
    tagMOTION* Get_MOTION(short nActionIdx = MOB_ANI_STOP) {
        return m_pCharMODEL->GetMOTION((BYTE)nActionIdx);
    }
    classUSER* Give_EXP();
...
public:
    CObjMOB() { m_SavedDAMAGED = NULL; }
    virtual ~CObjMOB() { SAFE_DELETE_ARRAY(m_SavedDAMAGED); }
    short m_nQuestIDX;
    int m_iOriMaxHP;
    ...
    bool Init(CZoneTHREAD* pZONE,
        short nCharIdx,
        float fXPos,
        float fYPos,
        int iTeamNO,
        CRegenPOINT* pRegenPOINT = NULL,
        short nQuestIDX = 0);
    int Proc();
    ...
    inline void Do_DeadAI(CObjCHAR* pTarget, int iDamage) {
...
public:
    ...
    // 필요..
    void SetCallerOBJ(int iCallerOBJ,
        t_HASHKEY HashCallerOBJ,
        int iCallerLEVEL,
        short nSkillIDX,
        CObjCHAR* pOwnerAVT);
...
public:
    ...
    void Set_TRIGGER(t_HASHKEY HashNextTrigger) { this->Set_NextTRIGGER(1, HashNextTrigger); }
