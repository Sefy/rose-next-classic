...
protected:
    ...
    void Add_EFFECT(CMODEL<CCharPART>* pCharPART,
        ...
        bool bLinkNODE = true);
    void LoadTRAIL(CMODEL<CCharPART>* pCharPART,
        short nPartIDX,
        bool bLinkBONE,
        int iColorNO,
        int iDuration,
...
public:
    ...
    /// @param bUseResetPosZ 갱신 높이값을 사용할지 여부. 높이값을 설정할 때에는 true, 초기화할
    /// 때에는 false를 입력한다.
    ...
    /*override*/ bool IsIntersect(float& fCurDistance) {
        return CGameOBJ::IsIntersect(m_hNodeMODEL, fCurDistance);
    }
    ...
    /*override*/ bool SetCMD_ATTACK(int iServerTarget) {
        return CObjAI::SetCMD_ATTACK(iServerTarget);
    }

    /*override*/ bool IsInRANGE(CObjCHAR* pT, int iRange) {
        return CD3DUtil::distance((int)m_PosCUR.x,
                   (int)m_PosCUR.y,
                   (int)pT->m_PosCUR.x,
                   (int)pT->m_PosCUR.y)
            <= iRange;
    ...
    /*override*/ void Set_MOTION(short nActionIdx,
        float fMoveSpeed = 0,
    ...
    /*override*/ virtual void SetMotionRepeatCount(int iRepeatCount) {
        ::setRepeatCount(GetZMODEL(), iRepeatCount);
    };
...
public:
    /*override*/ void ChangeActionMode(int iActionMode) {
        return m_ChangeActionMode.ChangeActionMode(iActionMode);
    }
    ...
    /*override*/ float Get_DISTANCE(CAI_OBJ* pObj) {
        return CD3DUtil::distance(m_PosCUR, ((CObjCHAR*)pObj)->m_PosCUR);
    }
...
public:
    ...
    /*override*/ virtual void
    PushCommandMove(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget);
    /*override*/ virtual void
    PushCommandAttack(int iServerTarget, WORD wSrvDIST, const D3DVECTOR& PosTO);
    ...
    /*override*/ virtual void
    PushCommandSkill2Obj(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget, short nSkillIDX);
...
public:
    ...
    virtual int Get_INT() {
        return 0;
    } /// !!!Cal 클래스에서 CObjCHAR 의 맴버로 접근하는부분이 있어 이것만..
...
public:
    ...
    void SetSCALE(float fScale) {
        m_fScale = fScale, ::setScale(m_hNodeMODEL, fScale, fScale, fScale);
    }
    ...
    bool Hitted(CObjCHAR* pFromOBJ,
        int iEffectIDX,
        int iSkillIDX = 0,
        bool bIsSkillEffect = false,
        bool bIsBulletHit = false,
        bool bJustEffect = false);
    ...
    } // 바닥에 위치한 오브젝트의 엔진 핸들을 리턴함. 지형이나, 물 등은 제외된다. 없으면 0.
...
public:
    ...
    /*override*/ void Do_AttackMoveAI(CObjCHAR* pTarget) {
    ...
    /*override*/ void Do_KillAI(CObjCHAR* pTarget, int iDamage) {
        g_AI_LIST.AI_Kill(NPC_AI_TYPE(m_nCharIdx), this, pTarget, iDamage);
    }
    /*override*/ void Do_DeadAI(CObjCHAR* pTarget, int iDamage) {
        g_AI_LIST.AI_Dead(NPC_AI_TYPE(m_nCharIdx), this, pTarget, iDamage);
    }
    ...
    /*override*/ tagMOTION* Get_MOTION(short nActionIdx = MOB_ANI_STOP) {
...
public:
    ...
    /*override*/ virtual bool Is_SameTYPE(int ObjTYPE) {
        return (ObjTYPE == OBJ_AVATAR) || (ObjTYPE == OBJ_USER);
    }
    ...
    /*override*/ virtual int Get_CON() {
        return m_iCon;
    } // 유저일 경우 return pAVATAR->GetCur_CON();
    /*override*/ virtual int GetAdd_RecoverHP() {
        return m_iRecoverHP;
    } // 유저일 경우 return pAVATAR->m_btRecoverHP;
    ...
    /*override*/ virtual void
    SetCMD_PET_MOVE(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget);
    /*override*/ virtual void SetCMD_PET_STOP(void);
    /*override*/ virtual void
    SetCMD_PET_ATTACK(int iServerTarget, WORD wSrvDIST, const D3DVECTOR& PosTO);
...
public:
    ...
    /// @pReachable 도달 가능한 위치인지의 여부. 이전 프레임의 위치로부터 도달가능하지 않다면, false
    /// 값이 세팅됨.
...
public:
    ...
    bool CreateCart(unsigned int iPetType,
        int iEnginePart,
        int iBodyPart,
        int iLegPart,
        int iAbilIPart,
