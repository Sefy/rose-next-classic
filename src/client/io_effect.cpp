...
bool
CParticle::RegisterToNZIN(char* szName, char* szParticleFile) {
...
void
CParticle::UnregisterFromNZINE(void) {
    if (m_hParticle) {
...
bool
CMeshEffect::RegisterToNZIN(HNODE hMesh, HNODE hMotion, HNODE hMaterial) {
    m_hVisible = ::loadVisible(NULL, NULL, NULL, NULL);
    if (!m_hVisible)
        return false;
    ...
    m_hMorpher = ::loadMorpher(NULL,
        hMesh,
        hMotion,
        hMaterial,
        NULL); // At this moment, MORPHER does not need to use g_GameDATA.m_hLight.
    if (!m_hMorpher) {
        ::unloadVisible(m_hVisible);
        m_hVisible = NULL;
...
void
CMeshEffect::UnregisterFromNZINE(void) {
    if (m_hMorpher) {
...
bool
CEffect::Create(CEffectDATA* pEffectDATA) {
    if (NULL == pEffectDATA)
        return false;
    ...
    char* szName = CStr::Printf("EFT_%s_%d",
        (pEffectDATA->m_Name.Get() == NULL) ? "NULL" : pEffectDATA->m_Name.Get(),
        m_iEffectCnt);
    char* szName2 = CStr::Printf("EFT_%s_%d_R2",
        (pEffectDATA->m_Name.Get() == NULL) ? "NULL" : pEffectDATA->m_Name.Get(),
        m_iEffectCnt);
    ...
    for (iC = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++) {
        ...
        if (m_pEffectDATA->m_pParticle[iC].m_bUseAniFile) {
            ...
            if (hMotion) {
                ...
                ::setRepeatCount(m_pParticle[iC].GetPARTICLE(),
                    m_pEffectDATA->m_pParticle[iC].m_iAniLoopCnt);
            }
        }
    }
    ...
    if (pEffectDATA->m_iMeshAniCNT) {
        ...
        for (iC = 0; iC < m_pEffectDATA->m_iMeshAniCNT; iC++) {
            ...
            ::setMaterialBlendTypeCustom(hMat,
                m_pEffectDATA->m_pMeshAni[iC].m_iBlendSour,
            ...
            if (m_pEffectDATA->m_pMeshAni[iC].m_bUseAniFile) {
                ...
                if (hMotion) {
                    ...
                    ::setRepeatCount(m_pMeshEffect[iC].GetANIMATABLE(),
                        m_pEffectDATA->m_pMeshAni[iC].m_iAniLoopCnt);
                }
            }

            ::setStartDelay(m_pMeshEffect[iC].GetMORPHER(), m_pEffectDATA->m_pMeshAni[iC].m_iDelay);
            ::setRepeatCount(m_pMeshEffect[iC].GetMORPHER(),
                m_pEffectDATA->m_pMeshAni[iC].m_iRepeatCNT);
        }
    }
    return true;
}
...
void
CEffect::Rotation(D3DXVECTOR3& vec) {
    D3DXVECTOR3 vPos;
...
//-------------------------------------------------------------------------------------------------
t_HASHKEY
CEffectLIST::Get_IndexHashKEY(short nIndex) {
...
CEffect*
CEffectLIST::Add_EffectWithIDX(short nIndex, bool bAdd2LIST) {
    if (nIndex < 0 || nIndex >= m_nFileEffectCNT) {
        _ASSERT(0);
        return NULL;
    }
...
//-------------------------------------------------------------------------------------------------
CEffectDATA*
CEffectLIST::Get_EffectDATA(t_HASHKEY uiHashKEY) {
    if (0 == uiHashKEY)
        return NULL;
...
CEffect*
CEffectLIST::Add_EFFECT(t_HASHKEY uiHashKEY, bool bAdd2LIST) {
    if (0 == uiHashKEY)
        return NULL;
...
CEffect*
CEffectLIST::Get_EffectClassWithHashKEY(t_HASHKEY uiHashKEY) {
    return NULL;
}
