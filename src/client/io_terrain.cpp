...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::InitializeQuadPatch() {
...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::InputQuadPatchElement(short index,
    short level,
    int iquad_size,
    short nMapX,
    short nMapY) {
    ...
    InputQuadPatchElement(index * 4 + 2,
        next_level,
        next_iquad_size,
        nMapX + next_iquad_size,
        nMapY);
    InputQuadPatchElement(index * 4 + 3,
        next_level,
        next_iquad_size,
        nMapX + next_iquad_size,
        nMapY + next_iquad_size);
    InputQuadPatchElement(index * 4 + 4,
        next_level,
        next_iquad_size,
        nMapX,
        nMapY + next_iquad_size);
...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::LoadQuadPatch(CMAP* pMap) {
...
void
CQuadPatchManager::CalculateViewFrustumCulling() {
...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::CalculateQuadPatchCulling(short index) {
...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::InsertMemoQuadPatch(short index) {
...
//----------------------------------------------------------------------------------------------------
void
CQuadPatchManager::CalculateBottomNode(short index) {
...
//----------------------------------------------------------------------------------------------------
short
CQuadPatchManager::ViewCullingFunc(aabbBox* box) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (((buffer_vector[0] * buffer_vector2[0] + buffer_vector[1] * buffer_vector2[1]
                 + buffer_vector[2] * buffer_vector2[2])
                + m_ViewFrsutumEq[i][3])
            > 0)
...
short
CQuadPatchManager::ViewCullingFunc2(D3DVECTOR* min, D3DVECTOR* max) {
...
bool
CQuadPatchManager::IntersectRayOnOff(aabbBox* box) {
    float vMin[3], vMax[3];
...
void
CQuadPatchManager::SetExPatchTotal() {
...
void
CQuadPatchManager::SetExPatchBottom(int index) {
    ...
    for (i = m_quadPATCH[index].index_y[0]; i <= m_quadPATCH[index].index_y[1]; i += 1) {
        for (j = m_quadPATCH[index].index_x[0]; j <= m_quadPATCH[index].index_x[1]; j += 1) {
            ...
            if (pPatch->ExPatchEnable) {
                if ((m_quadPATCH[index].aabb.x[0] > pPatch->m_AABBMin.x)
                    || (m_quadPATCH[index].aabb.x[1] < pPatch->m_AABBMax.x)
                    || (m_quadPATCH[index].aabb.y[0] > pPatch->m_AABBMin.y)
                    || (m_quadPATCH[index].aabb.y[1] < pPatch->m_AABBMax.y)) {
...
void
CQuadPatchManager::SetExPatch(int index, CMAP_PATCH* pPatch, int patch_x, int patch_y) {
...
void
CQuadPatchManager::InsertMemoQuadExPatch(short index) {
...
void
CQuadPatchManager::InsertMemoQuadExPatch2(short index) {
...
void
CQuadPatchManager::GetViewFrustumEq(float viewEquation[6][4]) {
    int i;
...
void
CQuadPatchManager::CalculateRayCulling(short index) {
...
void
CQuadPatchManager::CalculateRayCullingBottomNode(short index) {
...
void
CQuadPatchManager::CalculatePickingPATCH() {
...
void
CQuadPatchManager::GetPickingRay(float Position[3], float Direction[3]) {
...
bool
CMAP_PATCH::IsIntersect(float& fCurMinDist, D3DXVECTOR3& PosOUT) {
    float fDistance;
    D3DXVECTOR3 PosPICK;
    D3DXVECTOR3 RayOrig;
    D3DXVECTOR3 RayDir;
...
bool
CMAP_PATCH::IsIntersect_test(float& fCurMinDist,
    D3DXVECTOR3& PosOUT,
    float Position[3],
    float Direction[3]) {
...
void
CMAP_PATCH::SetPOSITION(CMAP* pMAP, short nPatchXFromMap, short nPatchYFromMap) {
    m_pMAP = pMAP;
...
CMAP_PATCH::Register(short nWorldX, short nWorldY) {
    ...
    if (!g_GameDATA.m_bJustObjectLoadMode) {
        ...
        m_hTerrainBlock = ::loadTerrainBlockEx(szName,
            nWorldX * fPATCH_SIZE,
            (1 + nWorldY) * fPATCH_SIZE,
            GRID_COUNT_PER_PATCH_AXIS,
            1,
            TILETBL_PUTTYPE(m_nTileNO),
            (float*)m_fHEIGHT,
            m_aabb.z,
            iMAP_SIZE,
            nPATCH_SIZE,
...
bool
CMAP_PATCH::RegisterToNZIN(void) {
    if (m_bIsRegistered)
        return false;
    ...
    for (nY = 0; nY <= GRID_COUNT_PER_PATCH_AXIS; nY++)
        for (nX = 0; nX <= GRID_COUNT_PER_PATCH_AXIS; nX++) {
            m_fHEIGHT[(GRID_COUNT_PER_PATCH_AXIS - nY) * (GRID_COUNT_PER_PATCH_AXIS + 1) + nX] =
                ...
                m_pMAP->GetVertexHEIGHT(m_nPatchXFromMAP * GRID_COUNT_PER_PATCH_AXIS + nX,
                    m_nPatchYFromMAP * GRID_COUNT_PER_PATCH_AXIS + nY);
        }
...
bool
CMAP_PATCH::UnregisterFromNZIN(void) {
    if (m_bIsRegistered) {
...
/// 그런데 지울때는 이펙트 리스트에서 지워주기를 바랬음
/// 그래서 다른 존으로 이동후에 이펙트들이 남았음..
/// 04/4/26 이건 m_pEffect->RemoveFromScene(); 했어야 했는데 지웠었군.. 그래서.. UnRegister 할때
/// assert에 걸렸다.
void
CMAP_PATCH::RemoveFromScene(void) {
    if (m_bIsVisible) {
        ///
        /// 하늘 연출등에서 지형블락은 로드를 하지 않아야 할 모드에서
        ///
...
CMAP_PATCH::PlaySOUND() {
    ...
    while (pSndNode) {
        ...
        if (dwTime - pSndNode->DATA.m_dwLastPlayTIME > pSndNode->DATA.m_uiInterval) {
            ...
            if (fMaxDistance < fMaxStart)
                fMaxDistance = fMaxStart;
            if (fMaxDistance > fMaxEnd)
                fMaxDistance = fMaxEnd;
            fMinDistance = fMaxDistance / 10;
            if (fMinDistance < fMinStart)
                fMinDistance = fMinStart;

            // LogString(LOG_DEBUG_, "Range = %d, Min(%f), Max(%f)\n", pSndNode->DATA.m_uiRange,
            // fMinDistance, fMaxDistance);

            g_pSoundLIST->KEY_PlaySound3D(pSndNode->DATA.m_HashKEY,
                pSndNode->DATA.m_Position,
                iVolume,
                0,
                0,
                fMinDistance,
...
bool
CMAP_PATCH::AddObject(CGameOBJ* pObject,
    D3DVECTOR& Position,
...
bool
CMAP_PATCH::MakeAABBFromObject(CGameOBJ* pObject,
    D3DVECTOR& Position,
    D3DXQUATERNION& Rotate,
    D3DVECTOR& Scale) {
...
void
CMAP_PATCH::TransformOBB2AABB(float* obb_min,
    float* obb_max,
    D3DXMATRIX* object_m,
    float* bb_min,
    float* bb_max) {
...
void
CMAP_PATCH::CompareSizePath2Obj() {
...
void
CMAP::AddEffect(D3DVECTOR& Position, char* szEffectFile) {
    short nPatchX, nPatchY;

    nPatchX = (short)(Position.x - (m_nZoneMapXIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
        / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
    nPatchY = (short)(Position.y - (m_nZoneMapYIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
        / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
...
//-------------------------------------------------------------------------------------------------
void
CMAP::AddSound(D3DVECTOR& Position,
    char* szSoundFile,
...
//-------------------------------------------------------------------------------------------------
/// 맵에선 31_31에 의 1,1 에 등록되었던 패치라도 여기서 실제 오브젝트 위치로.. 게임 좌표계로
/// 변환후의 패치에 새로 등록한다.( 새로운 좌표축 구성과 오브젝트 배치와의 비밀이 여기에.. 상당히
/// 복잡하다. )
bool
CMAP::AddObject(int iObjectIndex,
    ...
    if (pObject) {
        ...
        nPatchX = (short)(pObject->Get_CurPOS().x
                      - (m_nZoneMapXIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
            / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
        nPatchY = (short)(pObject->Get_CurPOS().y
                      - (m_nZoneMapYIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
            / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
        ...
        if (pObject->IsA(OBJ_GROUND)) {
            ...
            if (pModel->m_iCylinderRadius != 0) {
                ...
                nPatchMin[0] =
                    (short)(vMin.x - (m_nZoneMapXIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
                    / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
                nPatchMin[1] =
                    (short)(vMin.y - (m_nZoneMapYIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
                    / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
                nPatchMax[0] =
                    (short)(vMax.x - (m_nZoneMapXIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
                    / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
                nPatchMax[1] =
                    (short)(vMax.y - (m_nZoneMapYIDX * nGRID_SIZE * GRID_COUNT_PER_MAP_AXIS))
                    / (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS);
        ...
        if (m_PATCH[nPatchY][nPatchX].AddObject(pObject, Position, Rotate, Scale)) {
            /// 패치의 바운딩 박스가 변경되었다.
            if (std::find(m_PatchUpdateList.begin(),
                    m_PatchUpdateList.end(),
                    &m_PATCH[nPatchY][nPatchX])
                != m_PatchUpdateList.end())
...
// 단순 지형 높이를 얻는다.
float
CMAP::GetHEIGHT(float fWorldX, float fWorldY) {
    float fHeights[4];
...
/// 단순 지형 기울기를 얻는다.
float
CMAP::GetSlope(float fWorldX, float fWorldY) {
    float fHeights[4];
...
void
CMAP::GetNormal(float fWorldX, float fWorldY, D3DXVECTOR3& vNormal) {
    float fHeights[4];
    float fGridSize(fGRID_SIZE);
...
//-------------------------------------------------------------------------------------------------
void
CMAP::ReadMapINFO(CFileSystem* pFileSystem, long lOffset) {
    // 읽을 필요 없다.
    /*
...
//-------------------------------------------------------------------------------------------------
void
CMAP::ReadOceanINFO(CFileSystem* pFileSystem, long lOffset) {
    ...
    for (int iC = 0; iC < iCount; iC++) {
        ...
        fSX += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        fSY += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        fEX += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        fEY += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        fW = (fEX - fSX);
        fH = (fEY - fSY);
        ...
        hOceanBlock =
            ::loadOceanBlock(CStr::Printf("@OB%dx%d:%d", m_nZoneMapXIDX, m_nZoneMapYIDX, iC),
                fSX,
                fSY,
                fSZ,
                fW,
...
//-------------------------------------------------------------------------------------------------
void
CMAP::ReadObjINFO(CFileSystem* pFileSystem, long lOffset, int iLumpType) {
    pFileSystem->Seek(lOffset, FILE_POS_SET);
        ...
        switch (iLumpType) {
            ...
            case LUMP_TERRAIN_MOB: {
                ...
                if (NPC_TYPE(iObjID) == 999) {
                    ...
                    iObjectIndex =
                        g_pObjMGR->Add_NpcCHAR(0, iObjID, Position, nQuestIDX, fModelDIR);
            ...
            case LUMP_TERRAIN_EVENT_OBJECT: {
                ...
                iObjectIndex = g_pObjMGR->Add_FixedEvent(iEventID,
                    iObjID,
...
//-------------------------------------------------------------------------------------------------
bool
CMAP::LoadLightMapINFO(int iLumpType, char* szLightFileINF, char* szPath) {
...
CMAP::Load(char* szFileName, short nZoneMapXIDX, short nZoneMapYIDX) {
    ...
    szFullPathName = CStr::Printf("%s\\%d_%d_PlaneLightingMap.dds",
        szFileName,
        nZoneMapXIDX,
        MAP_COUNT_PER_ZONE_AXIS - nZoneMapYIDX);

    if ((CVFSManager::GetSingleton()).IsExistFile(szFullPathName) == false) {
        szFullPathName = CStr::Printf("%s\\%d_%d_PlaneLightingMap.tga",
            szFileName,
            nZoneMapXIDX,
            MAP_COUNT_PER_ZONE_AXIS - nZoneMapYIDX);
    ...
    m_hLightMAP =
        ::loadColormapMaterial(CStr::Printf("%dx%d_%s", nZoneMapXIDX, nZoneMapYIDX, szFullPathName),
            g_GameDATA.m_hShader_lightmap,
            szFullPathName);
    if (m_hLightMAP) {
        ;
...
void
CMAP::ClearFromNZIN(void) {
    if (this->IsEmpty())
        return; // 추가된 코드 by zho
...
POINT
CMAP::GetMapIndexFromWorld(float fWorldX, float fWorldY) {
...
void
CMAP::GetHeightsByCoordinates(float fWorldX,
    float fWorldY,
    float fHeights[4],
    float* pfWeightHoriz,
    float* pfWeightVert) {
    ...
    if (pfWeightHoriz)
        *pfWeightHoriz = ((fWorldX - iHoriz * nGridSize) / fGridSize);
    if (pfWeightVert)
        *pfWeightVert = ((fWorldY - iVerti * nGridSize) / fGridSize);
...
///
CMAP_PATCH*
CMAP::GetPatchFromWorld(float fWorldX, float fWorldY) {
...
tPOINTF
CTERRAIN::Get_EventPOS(char* szName) {
    tPOINTF OutPOS;
...
tPOINTF
CTERRAIN::Get_StartPOS() {
    return Get_EventPOS(ZONE_START_POS(m_nZoneNO));
}
tPOINTF
CTERRAIN::Get_RevivePOS() {
    return Get_EventPOS(ZONE_REVIVE_POS(m_nZoneNO));
}

void
CTERRAIN::Add_RegenAREA(classDLLNODE<CRegenAREA>* pNode) {
    m_RegenLIST.AppendNode(pNode);
}
void
CTERRAIN::Del_RegenAREA(classDLLNODE<CRegenAREA>* pNode) {
    m_RegenLIST.DeleteNode(pNode);
}
void
CTERRAIN::Proc_RegenAREA(void) {
...
//-------------------------------------------------------------------------------------------------
HNODE
CTERRAIN::GetLIGHTMAP(char* szLightMapFile) {
...
//-------------------------------------------------------------------------------------------------
char*
CTERRAIN::GetMapFILE(short nMapX, short nMapY) {
    ...
    char* szMapFile =
        CStr::Printf("%s%d_%d", m_ZoneDIR.Get(), nMapX, MAP_COUNT_PER_ZONE_AXIS - nMapY);
...
//-------------------------------------------------------------------------------------------------
void
CTERRAIN::ReadZoneINFO(CFileSystem* pFileSystem, long lOffset) {
    int iValue;
    pFileSystem->Seek(lOffset, FILE_POS_SET);
...
void
CTERRAIN::ReadEventObjINFO(CFileSystem* pFileSystem, long lOffset) {
    pFileSystem->Seek(lOffset, FILE_POS_SET);
    ...
    for (int iC = 0; iC < iEventCNT; iC++) {
        ...
        m_pEventPOS[iC].m_Position.x += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
        m_pEventPOS[iC].m_Position.y += (MAP_COUNT_PER_ZONE_AXIS / 2
                * (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS)
            + (nGRID_SIZE * GRID_COUNT_PER_PATCH_AXIS * PATCH_COUNT_PER_MAP_AXIS) / 2);
...
void
CTERRAIN::ReadTileINFO(CFileSystem* pFileSystem, long lOffset) {
...
bool
CTERRAIN::ReadECONOMY(CFileSystem* pFileSystem, long lOffset) {
    pFileSystem->Seek(lOffset, FILE_POS_SET);
...
bool
CTERRAIN::LoadZONE(short nZoneNO, bool bPlayBGM) {
    ...
    if (bPlayBGM) {
        if (CMusicMgr::GetSingleton().bIsReady()) {
            ...
            CMusicMgr::GetSingleton().SetVolume(
...
//-------------------------------------------------------------------------------------------------
bool
CTERRAIN::InitZONE(float fCenterX, float fCenterY) {
    this->SubMAP(MOVE_UPDATE_ALL);
...
CTERRAIN::GetPATCH(int iZonePatchX, int iZonePatchY) {
    ...
    if (pMAP)
        return pMAP->GetPATCH(iZonePatchX % PATCH_COUNT_PER_MAP_AXIS,
            iZonePatchY % PATCH_COUNT_PER_MAP_AXIS);
...
float
CTERRAIN::GetHeight(float fWorldX, float fWorldY, bool* pNotFound) {
...
float
CTERRAIN::GetHeightTop(float fWorldX, float fWorldY, bool* pNotFound) {
    ...
    return (fHeight > fTerrainHeight)
        ? fHeight
        : fTerrainHeight; // 지형 높이와 오브젝트 높이 중 더 높은 것 선택
}

CMAP*
CTERRAIN::GetMapPtrByWorldCoordinates(float fWorldX, float fWorldY) {
    ...
    if (nZoneMapX >= MAP_COUNT_PER_ZONE_AXIS || nZoneMapY >= MAP_COUNT_PER_ZONE_AXIS) {
        _ASSERT(
            0 && "nZoneMapX >= MAP_COUNT_PER_ZONE_AXIS || nZoneMapY >= MAP_COUNT_PER_ZONE_AXIS");
        return NULL;
    }
...
float
CTERRAIN::GetSlope(float fWorldX, float fWorldY, bool* pNotFound) {
    CMAP* pMAP = GetMapPtrByWorldCoordinates(fWorldX, fWorldY);
...
bool
CTERRAIN::GetNormal(float fWorldX, float fWorldY, D3DXVECTOR3& vNormal) {
    CMAP* pMAP = GetMapPtrByWorldCoordinates(fWorldX, fWorldY);
...
//-------------------------------------------------------------------------------------------------
float
CTERRAIN::Pick_POSITION(D3DXVECTOR3& PosPICK) {
    return m_PatchManager.Pick_POSITION(PosPICK);
}

//-------------------------------------------------------------------------------------------------
float
CTERRAIN::Pick_POSITION_Test(D3DXVECTOR3& PosPICK) {
...
short
CTERRAIN::Pick_NearObject(D3DXVECTOR3& PosPICK, float& fDistance) {
    CGameOBJ* pObj = NULL;
    HNODE hNode = 0;
    HNODE hUserData = 0;
    int iPickObject = 0;
...
short
CTERRAIN::Pick_OBJECT(D3DXVECTOR3& PosPICK, float& fDistance) {
    //	PosPICK.x;
    //	PosPICK.y;
...
CMAP*
CTERRAIN::FindLoadedMAP(short nZoneMapXIDX, short nZoneMapYIDX) {
    for (short nI = 0; nI < MAX_MAP_BUFFER; nI++) {
        if (m_pMapBUFFER[nI]->IsEmpty())
            continue;
...
CMAP*
CTERRAIN::GetMapPTR(short nZoneMapX, short nZoneMapY) {
    if (nZoneMapX < 0 || nZoneMapX >= MAP_COUNT_PER_ZONE_AXIS)
        return NULL;
    if (nZoneMapY < 0 || nZoneMapY >= MAP_COUNT_PER_ZONE_AXIS)
        return NULL;

#ifdef _DEBUG
    if (NULL == m_pMAPS[nZoneMapY][nZoneMapX]) {
        LogString(LOG_DEBUG_,
            " GetMapPTR:: Map[ %d, %d ] is NULL Filename( %s ) !!!!\n",
            nZoneMapX,
            nZoneMapY,
            this->GetMapFILE(nZoneMapX, nZoneMapY));
    }
#endif
...
void
CTERRAIN::SetMapPTR(short nZoneMapX, short nZoneMapY, CMAP* pMAP) {
    ...
    if (m_pMAPS[nZoneMapY][nZoneMapX]) {
        ...
        LogString(LOG_DEBUG_,
            " SetMapPTR:: Map[ %d, %d ] is NULL Filename !!!!\n",
            nZoneMapX,
            nZoneMapY);
    }
#endif
...
///
void
CTERRAIN::SubMAP(WORD wUpdateFLAG) {
    ...
    for (nI = 0; s_SubSecIdx[wUpdateFLAG][nI] >= 0; nI++) {
        ...
        if (nZoneMapXIDX >= 0 && nZoneMapXIDX < MAP_COUNT_PER_ZONE_AXIS && nZoneMapYIDX >= 0
            ...
            if (pMAP) {
                LogString(LOG_DEBUG_,
                    "		FreeMAP: (%d, %d) --> ( %d, %d )\n",
                    s_SecAdjPos[s_SubSecIdx[wUpdateFLAG][nI]].m_nX,
                    s_SecAdjPos[s_SubSecIdx[wUpdateFLAG][nI]].m_nY,
                    nZoneMapXIDX,
                    nZoneMapYIDX);
                pMAP->Free();
                ...
                /// 점진적 로딩 리스트에 들어있다면 빼라...
                {
                    ...
                    for (; begin != m_LoadOneMapData.end();) ///<= 2004 / 2 / 2 : nAvy 수정
                    {
                        ...
                        short nZoneXIDX =
                            pOneMapData->m_nCenterX + s_SecAdjPos[pOneMapData->m_nUpdateIndex].m_nX;
                        short nZoneYIDY =
                            pOneMapData->m_nCenterY + s_SecAdjPos[pOneMapData->m_nUpdateIndex].m_nY;
                        ...
                        { /// if( --begin == end() )일경우 erase 하고 ++begin하면 부적절한 iter가
                          /// 된다.
                            ++begin; ///
                        } ///
                    }
...
CTERRAIN::GetLodingMapFromList(short nCenterMapXIDX, short nCenterMapYIDX, WORD wUpdateIndex) {
...
///
bool
CTERRAIN::AddOneMap(short nCenterMapXIDX, short nCenterMapYIDX, WORD wUpdateIndex) {
    ...
    if (nZoneMapXIDX >= 0 && nZoneMapXIDX < MAP_COUNT_PER_ZONE_AXIS && nZoneMapYIDX >= 0
        ...
        if (pMAP) {
            ...
            if (pMAP) {
                ...
                if (szMapFile) {
                    this->SetMapPTR(nZoneMapXIDX,
                        nZoneMapYIDX,
                        pMAP->Load(szMapFile, nZoneMapXIDX, nZoneMapYIDX));
                } else {
                    // 맵없다.
...
///
bool
CTERRAIN::AddMAP(short nCenterMapXIDX,
    short nCenterMapYIDX,
    WORD wUpdateFLAG,
    bool bAllUpdate,
    bool bImmediateLoad) {
    short nI, nZoneMapXIDX, nZoneMapYIDX;
    ...
    if (!bImmediateLoad) {
        ...
        for (nI = 0; s_AddSecIdx[wUpdateFLAG][nI] >= 0; nI++) {
            ...
            tagLOAD_ONE_MAP_DATA* pOneMapData =
                GetLodingMapFromList(nCenterMapXIDX, nCenterMapYIDX, s_AddSecIdx[wUpdateFLAG][nI]);
            if (pOneMapData) {
    ...
    /// 다이렉트 로딩
    {
        ...
        for (nI = 0; s_AddSecIdx[wUpdateFLAG][nI] >= 0; nI++) {
            ...
            if (nZoneMapXIDX >= 0 && nZoneMapXIDX < MAP_COUNT_PER_ZONE_AXIS && nZoneMapYIDX >= 0
                ...
                LogString(LOG_DEBUG_,
                    "AddMAP: (%d, %d) --> ( %d, %d ) \n",
                    s_SecAdjPos[s_AddSecIdx[wUpdateFLAG][nI]].m_nX,
                    s_SecAdjPos[s_AddSecIdx[wUpdateFLAG][nI]].m_nY,
                    nZoneMapXIDX,
                    nZoneMapYIDX);
...
void
CTERRAIN::SetCenterPosition(float fWorldX, float fWorldY) {
...
void
CTERRAIN::ReOrginazationPatch(short nZoneMapX, short nZoneMapY) {
    short nX, nZ;
...
///
CMAP_PATCH*
CTERRAIN::GetPatchFromWorld(float fWorldX, float fWorldY) {
    int iWorldX = (int)fWorldX;
    int iWorldY = (int)fWorldY;
...
void
CTERRAIN::UpdatePatchManager(short nCenterMapXIDX, short nCenterMapYIDX) {
...
void
CTERRAIN::InsertCameraPatch(float xChar, float yChar) {
    ...
    for (nIndex = g_pCRange->GetStartIndex(0); nIndex < g_pCRange->GetStartIndex(4); nIndex++) {
        ...
        if (m_pMAPS[map_center[1]][map_center[0]] != NULL
            && m_pMAPS[map_center[1]][map_center[0]]->IsUsing()) {
            patch_position[0] = nX - 16 * map_center[0];
            patch_position[1] = nY - 16 * map_center[1];

            m_PatchManager.InsertCameraPatch(
                m_pMAPS[map_center[1]][map_center[0]]->GetPATCH(patch_position[0],
                    patch_position[1]));
        }
    }
