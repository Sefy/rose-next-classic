...
int g_iFindDIST;
CAI_OBJ*
AI_FindFirstOBJ(CAI_OBJ* pBaseOBJ, int iDistance) {
...
bool
CObjectMANAGER::Set_EmptySlot(short nSlotNO, WORD wServerObjectIndex, CGameOBJ* pObject) {
...
void
CObjectMANAGER::Del_Object(short nIndex) {
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_GndITEM(WORD wServerObjectIndex,
    tagITEM& sITEM,
    tPOINTF& PosXY,
    float fRefZPos,
    bool bUseRefZPos) {
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_MORPHER(short nObjID,
    D3DVECTOR& Position,
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_GndTREE(short nTreeIdx,
    D3DVECTOR& Position,
    D3DXQUATERNION& Rotate,
    D3DVECTOR& Scale) {
    int iObjSlot;
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_GndCNST(short nCnstIdx,
    D3DVECTOR& Position,
    D3DXQUATERNION& Rotate,
    D3DVECTOR& Scale) {
    int iObjSlot;
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_MobCHAR(WORD wServerObjectIndex,
    short nCharIdx,
...
/// Warp object 추가
int
CObjectMANAGER::Add_WARP(short nEventID,
    D3DVECTOR& Position,
    D3DXQUATERNION& Rotate,
    D3DVECTOR& Scale) {
    int iObjSlot;
...
/// Collision object 추가
int
CObjectMANAGER::Add_CollisionBox(int iReserved,
    D3DVECTOR& Position,
    D3DXQUATERNION& Rotate,
    D3DVECTOR& Scale) {
    int iObjSlot;
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_NpcCHAR(WORD wServerObjectIndex,
    short nCharIdx,
    const D3DVECTOR& Position,
    short nQuestIDX,
    float fModelDIR) {
...
//----------------------------------------------------------------------------------------------------
/// @param
/// @brief user char, avt char 공통으로 여기서 생성을 하지 않기 때문에 모델에 userdata 세팅이
/// 불가능하다.
//----------------------------------------------------------------------------------------------------

CObjUSER*
CObjectMANAGER::New_UserCHAR(WORD wServerObjectIndex, char* szName) {
    int iObjSlot;
...
//----------------------------------------------------------------------------------------------------
CObjAVT*
CObjectMANAGER::New_AvtCHAR(WORD wServerObjectIndex, char* szName) {
    int iObjSlot;
...
//-------------------------------------------------------------------------------------------------
int
CObjectMANAGER::Add_FixedEvent(int iEventID,
    short nObjID,
...
//-------------------------------------------------------------------------------------------------
bool
CObjectMANAGER::Add_AvtCHAR(CObjAVT* pAvtCHAR, D3DVECTOR& Position, BYTE btCharRACE) {
    if (NULL == pAvtCHAR)
        return false;
...
//-------------------------------------------------------------------------------------------------
CGameOBJ*
CObjectMANAGER::Get_OBJECT(int iObjectIndex, t_ObjTAG ObjTYPE) {
...
//-------------------------------------------------------------------------------------------------
CObjCHAR*
CObjectMANAGER::Get_CharOBJ(int iObjectIndex, bool bCheckHP) {
    if (iObjectIndex >= MIN_OBJECTS && iObjectIndex < MAX_CLIENT_OBJECTS) {
...
CObjCHAR*
CObjectMANAGER::Get_ClientCharOBJByName(const char* szName) {
    if (szName == NULL)
        return NULL;
...
//-------------------------------------------------------------------------------------------------
CObjAVT*
CObjectMANAGER::Get_CharAVT(int iObjectIndex, bool bCheckHP) {
    if (iObjectIndex >= MIN_OBJECTS && iObjectIndex < MAX_CLIENT_OBJECTS) {
        CObjCHAR* pObject = (CObjCHAR*)g_pObjMGR->m_pOBJECTS[iObjectIndex];
        if (pObject) {
            if (pObject->IsUSER()) {
...
//-------------------------------------------------------------------------------------------------
CObjITEM*
CObjectMANAGER::Get_ItemOBJ(int iObjectIndex) {
    if (iObjectIndex >= MIN_OBJECTS && iObjectIndex < MAX_CLIENT_OBJECTS) {
...
//-------------------------------------------------------------------------------------------------
char*
CObjectMANAGER::Get_NAME(int iTargetObjectIndex) {
...
void
CObjectMANAGER::RemoveFromScene() {
    for (int iO = MIN_OBJECTS; iO < MAX_CLIENT_OBJECTS; iO++) {
        if (NULL == m_pOBJECTS[iO])
...
void
CObjectMANAGER::AddNpcInfo(CMAP* pMap, int iNpcNo, D3DVECTOR Position) {
    NpcInfoInMap Info;
    Info.m_pMap = pMap;
...
//--------------------------------------------------------------------------------
void
CObjectMANAGER::RemoveNpcInfo(CMAP* pMap) {
...
//--------------------------------------------------------------------------------
void
CObjectMANAGER::ClearNpcInfoList() {
    m_listNpcInfo.clear();
}
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
const std::list<NpcInfoInMap>&
CObjectMANAGER::GetNpcInfoList() {
...
//--------------------------------------------------------------------------------
CObjFixedEvent*
CObjectMANAGER::GetEventObject(int iEventID) {
...
//--------------------------------------------------------------------------------
void
CObjectMANAGER::ResetClanMarkInfo(DWORD dwClanID, WORD crc16) {
    classDLLNODE<int>* pNode;
    pNode = m_CharLIST.GetHeadNode();
    CObjAVT* pAvatar;
    while (pNode) {
