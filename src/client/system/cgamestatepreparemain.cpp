...
int
CGameStatePrepareMain::Update(bool bLostFocus) {
...
int
CGameStatePrepareMain::Leave(int iNextStateID) {
    ...
    g_pAVATAR->SetClan(Clan.GetClanNo(),
        Clan.GetMarkBack(),
        Clan.GetMarkCenter(),
        Clan.GetName(),
        Clan.GetLevel(),
...
unsigned __stdcall CGameStatePrepareMain::ThreadFunc(void* pArguments) {
    ...
    if (pDlg)
        pDlg->SetMinimap(ZONE_MINIMAP_NAME(Reply.m_nZoneNO),
            "3DData\\Control\\Res\\MiniMap_Arrow.tga",
            ZONE_MINIMAP_STARTX(Reply.m_nZoneNO),
            ZONE_MINIMAP_STARTY(Reply.m_nZoneNO));
