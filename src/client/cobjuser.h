...
private:
    /*override*/ void SetCur_PartITEM(short nPartIdx, tagITEM& sITEM) {
        this->SetPartITEM(nPartIdx, sITEM);
    }
...
public:
    ...
    /*override*/ int GetCur_AVOID() {
        return GetDef_AVOID() + m_EndurancePack.GetStateValue(ING_INC_AVOID)
            - m_EndurancePack.GetStateValue(ING_DEC_AVOID);
    }
...
public:
    ...
    }; /// 현재 존에 소환되어 있는 NPC갯수(죽을때, 존 워프시 0으로 리셋함)
