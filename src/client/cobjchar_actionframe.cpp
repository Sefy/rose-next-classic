...
void
CObjCHAR::ActionGun(int iActionIDX) {
    CObjCHAR* pTarget = g_pObjMGR->Get_CharOBJ(this->m_iActiveObject, true);
    ...
    if (iEffectIDX) {
        ...
        if (pCastingEFT) {
            ...
            if (SKILL_CASTING_EFFECT_POINT(iSkillIDX, iCastingOrder) == INVALID_DUMMY_POINT_NUM) {
                ...
                LinkDummy(pCastingEFT->GetZNODE(),
...
void
CObjCHAR::FireEffectBullet(CObjCHAR* pTarget, int iBulletIDX, bool bDummy, int iSkillIDX) {
    if (pTarget && iBulletIDX) {
        if (!bDummy)
            g_pBltMGR->Add_BULLET(this,
                pTarget,
                iBulletIDX,
                (iSkillIDX == 0),
                0,
