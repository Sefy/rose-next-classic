...
tagMOTION*
CCharMODEL::GetMOTION(short nIndex) {
    tagMOTION* pData = g_MotionFILE.KEY_GetMOTION(m_BoneAniFILE[nIndex]);

    if (NULL == pData) {
#ifdef _DEBUG
        LogString(LOG_DEBUG_,
            "(%s) Motion ERROR :: %s [ %d / %d ] \n",
            m_Name.Get(),
            g_MotionFILE.Get_FileNAME(m_BoneAniFILE[nIndex]),
            m_BoneAniFILE[nIndex],
            nIndex);
#else
        LogString(LOG_DEBUG_,
            "Motion ERROR :: %s [ %d / %d ] \n",
...
//-------------------------------------------------------------------------------------------------
bool
CCharMODEL::SetPartMODEL(short nPartIdx, CMODEL<CCharPART>* pPartMODEL) {
...
bool
CCharMODEL::SetPartMODEL(short nPartIdx, short nModelIdx) {
...
//-------------------------------------------------------------------------------------------------
void
CCharMODEL::SetMeshMOTION(HNODE** pphPartVIS, short nAniIdx) {
    HNODE hMotion;
    ...
    for (nB = 0; nB < MAX_BODY_PART; nB++) {
        ...
        for (nU = 0; nU < m_pBodyPART[nB]->m_nPartCNT; nU++) {
            if (m_pBodyPART[nB]->m_pParts[nU].m_pMeshAniFILE) {
                hMotion = g_MotionFILE.KEY_GetZMOTION(
                    m_pBodyPART[nB]->m_pParts[nU].m_pMeshAniFILE[nAniIdx]);
                if (hMotion) {
...
//-------------------------------------------------------------------------------------------------
void
CCharMODEL::DeletePART(short nPartIdx, HNODE* pVIS) {
    if (NULL != pVIS) {
...
void
CCharMODEL::ClearRenderUnitParts() {
...
//-------------------------------------------------------------------------------------------------
bool
CCharMODEL::Load_MOBorNPC(CFileSystem* pFileSystem,
    t_HASHKEY* pSkelKEY,
    t_HASHKEY* pAniKEY,
    t_HASHKEY* pEftKEY) {
    char cIsValid;
...
//-------------------------------------------------------------------------------------------------
CEffect**
CCharMODEL::CreateBoneEFFECT(HNODE hMODEL) {
    //	return NULL;
    if (m_nBoneEFFECT < 1)
        return NULL;
...
/// @Bug g_pEffectLIST->Add_EFFECT( .., default false ) 로 추가한 이펙트는 리스트에 추가되지
/// 않기때문에 직접 지워야한다.
///
///
void
...
void
CCharMODEL::LinkBoneEFFECT(HNODE hMODEL, CEffect** ppEffect) {
    for (short nE = 0; nE < m_nBoneEFFECT; nE++) {
        int iDummyCnt = ::getNumDummies(hMODEL);
        if (iDummyCnt >= m_pBoneEFFECT[nE].m_cBoneIDX)
            ::linkDummy(hMODEL, ppEffect[nE]->GetZNODE(), m_pBoneEFFECT[nE].m_cBoneIDX);
...
void
CCharMODEL::UnlinkBoneEFFECT(CEffect** ppEffect) {
    for (short nE = 0; nE < m_nBoneEFFECT; nE++) {
...
bool
CCharModelDATA::Load_MOBorNPC(char* szFileName) {
