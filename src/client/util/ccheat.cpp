...
bool
DoCheatFunc(char* pStr) {
    return g_Cheat.DoCheat(pStr);
}

char* s_pLangStr[] = {"en", // 0; English
    "ar", // 1; Arabic
    "bg", // 2; BULGARIAN
    "nl", // 3; DUTCH
    "fr", // 4; FRENCH
...
void
classPARSESTR::CheatInit() {}

char*
classPARSESTR::ParseString(char* pStr, short nStrNo) {
...
short
classPARSESTR::ParseCheatCodeIdx(char* pStr) {
...
int
classPARSESTR::DoCheat(char* pCheatCode, bool bServerCheate) {
    char* pStr = ParseString(pCheatCode, 0);
    short nCodeIdx = ParseCheatCodeIdx(pStr);
...
short
classPARSESTR::GetSET_CodeIdx(char* pStr) {
...
short
classPARSESTR::GetADD_CodeIdx(char* pStr) {
...
short
classPARSESTR::GetFULL_CodeIdx(char* pStr) {
...
bool
classPARSESTR::DoSER(char* pStr) {
    return true;
}

bool
classPARSESTR::DoFAME(char* pStr) {
    return true;
}

bool
classPARSESTR::DoMEET(char* pStr) {
    return true;
}

bool
classPARSESTR::DoFLY_MODE(char* pStr) {
    return true;
}

bool
classPARSESTR::DoWARP(char* pStr) {
    char* pARG;
...
bool
classPARSESTR::DoWARP_M(char* pStr) {
    return true;
}
...
bool
classPARSESTR::DoSOUND(char* pStr) {
    return true;
}
...
classPARSESTR::DoProcVal(char* pStr) {
    ...
    // sprintf( Buf, "소속별 명성[ %d: %d ]", iUnionNO, g_pAVATAR->m_Quests.m_nQuestFameOfUnion[
    // iUnionNO ] );
...
classPARSESTR::DoUnionFame(char* pStr) {
    ...
    // sprintf( Buf, "진행수치[ %d : %d ]", iUnionNO, g_pAVATAR->m_Quests.m_nQuestProcOfUnion[
    // iUnionNO ] );
    g_itMGR.AppendChatMsg(g_MsgBuf, IT_MGR::CHAT_TYPE_SYSTEM);
...
classPARSESTR::DoQuestVal(char* pStr) {
    ...
    if (stricmp("VJ", pARG) == 0) {
        for (int i = 0; i < QUEST_JOB_VAR_CNT; ++i)
            g_itMGR.AppendChatMsg(
                CStr::Printf("[ JOB%d : %d ]", i + 1, g_pAVATAR->m_Quests.m_nJobVAR[i]),
    ...
    if (stricmp("VP", pARG) == 0) {
        for (int i = 0; i < QUEST_PLANET_VAR_CNT; ++i)
            g_itMGR.AppendChatMsg(
                CStr::Printf("[PLANET%d : %d ]", i + 1, g_pAVATAR->m_Quests.m_nPlanetVAR[i]),
                IT_MGR::CHAT_TYPE_SYSTEM);
    }
    ...
    if (stricmp("VE", pARG) == 0) {
        for (int i = 0; i < QUEST_EPISODE_VAR_CNT; ++i)
            g_itMGR.AppendChatMsg(
                CStr::Printf("[EPISODE%d : %d ]", i + 1, g_pAVATAR->m_Quests.m_nEpisodeVAR[i]),
                IT_MGR::CHAT_TYPE_SYSTEM);
    }
    ...
    if (stricmp("VU", pARG) == 0) {
        for (int i = 0; i < QUEST_UNION_VAR_CNT; ++i)
            g_itMGR.AppendChatMsg(
                CStr::Printf("[UNION%d : %d ]", i + 1, g_pAVATAR->m_Quests.m_nUnionVAR[i]),
                IT_MGR::CHAT_TYPE_SYSTEM);
    }
    ...
    if (strcmp("TIME", pARG) == 0) {
        sprintf(g_MsgBuf,
            "[ TIME : %d ]",
            g_pAVATAR->m_Quests.m_QUEST[iSlotNO].GetExpirationTIME());
        g_itMGR.AppendChatMsg(g_MsgBuf, IT_MGR::CHAT_TYPE_SYSTEM);
    }
...
bool
classPARSESTR::DoSpeed(char* pStr) {
    char* pARG;
    pARG = ParseString(pStr, 1);
...
bool
classPARSESTR::DoFree(char* pStr) {
    char* pARG;
...
bool
classPARSESTR::DoCheckQuestCondition(char* pszStr) {
    if (pszStr == NULL)
        return false;
...
bool
classPARSESTR::DoClan(char* pStr) {
...
classPARSESTR::DoFee() {
    ...
    switch (CGame::GetInstance().GetExpireTimeType()) {
        ...
        case 2: //분
        {
            ...
            g_itMGR.AppendChatMsg(CStr::Printf("사용시간이 %d분 남았습니다", remain_time),
                IT_MGR::CHAT_TYPE_SYSTEM);
        } break;
        default:
            break;
