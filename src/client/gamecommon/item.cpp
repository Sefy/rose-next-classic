...
void
CItem::SetCommand(CTCommand* pCmd) {
    m_pCommand = pCmd;
}
...
void
CItem::SetDefaultIcon(CIcon* pIcon) {
...
void
CItem::SetLastExecTime(DWORD dwTime) {
    m_dwLastExecTime = dwTime;
}
DWORD
CItem::GetLastExecTime() {
...
void
CItem::AddItemName(tagITEM& sItem, CInfo& Info) {
    DWORD dwTextColor = 0;
    ...
    if (sItem.GetGrade())
        ...
        Info.AddString(CStr::Printf("%s", ITEM_NAME(sItem.GetTYPE(), sItem.GetItemNO())),
            dwTextColor,
            g_GameDATA.m_hFONT[FONT_NORMAL_BOLD]);
}
...
void
CItem::AddItemPatAttackPowSpeed(tagITEM& sItem, CInfo& Info) {
    if (GetAttackPower(sItem) <= 0)
        return;
...
void
CItem::AddItemPatDesc(tagITEM& sItem, CInfo& Info) {
...
void
CItem::AddItemPatEquipRequireJob(tagITEM& sItem, CInfo& Info) {
    int iRequireClass = PAT_ITEM_EQUIP_REQUIRE_CLASS(sItem.GetItemNO());

    if (iRequireClass) {
        const char* pszTemp =
            CStr::Printf("[%s: %s]", STR_EQUIP_REQUIRE_JOB, CLASS_NAME(iRequireClass));
        DWORD color = g_dwRED;
...
void
CItem::GetToolTipRidePart_BattleCart(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
...
void
CItem::GetToolTip(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
    tagITEM& sItem = GetItem();
    ...
    // if( CGame::GetInstance().IsRepairMode() && dwDialogType == DLG_TYPE_ITEM &&
    // sItem.IsEquipITEM() )
    //{
    ...
    switch (nItemType) {
        case ITEM_TYPE_WEAPON: {
            ...
            if (sItem.GetGrade() <= 0)
                ...
                strTemp.append(
                    CStr::Printf("%d", sItem.GetHitRate() + ITEMGRADE_HIT(sItem.GetGrade())));
                    ...
                    if (GEMITEM_ADD_DATA_TYPE(nItemIdx, i) == AT_SAVE_MP) {
                        ToolTip.AddString(CStr::Printf("[%s %d%%]",
                                              CStringManager::GetSingleton().GetAbility(
                                                  GEMITEM_ADD_DATA_TYPE(nItemIdx, i)),
                        ...
                        } else ///그외 스킬사용 매직아이템일경우
                        {
                            ...
                            for (int iUseProperty = 0; iUseProperty < SKILL_USE_PROPERTY_CNT;
                                ...
                                if (SKILL_USE_PROPERTY(iSkillNo, iUseProperty)) {
                                    iUseValue =
                                        g_pAVATAR->Skill_ToUseAbilityVALUE(iSkillNo, iUseProperty);

                                    if (g_pAVATAR->Get_AbilityValue(
                ...
                case USE_ITEM_MAINTAIN_ITEM: ///시간제 쿠폰 아이템
                    if (USEITEM_ADD_DATA_TYPE(nItemIdx))
                        ToolTip.AddString(CStr::Printf("[%s %d %s]",
                            CStringManager::GetSingleton().GetAbility(
                                USEITEM_ADD_DATA_TYPE(nItemIdx)),
                            USEITEM_ADD_DATA_VALUE(nItemIdx) / 144,
                            STR_DAY));
        ...
        case ITEM_TYPE_RIDE_PART: {
            ...
            if ((iClass % 100) / 10 == 5) /// WEAPON
            {
                ...
                if (sItem.GetGrade())
                    ToolTip.AddString(
                        CStr::Printf("%s (%d)", ITEM_NAME(nItemType, nItemIdx), sItem.GetGrade()),
                        dwTextColor,
                        g_GameDATA.m_hFONT[FONT_NORMAL_BOLD]);
                else
                    ToolTip.AddString(CStr::Printf("%s", ITEM_NAME(nItemType, nItemIdx)),
    ...
    if ((dwType & INFO_ADD_PRICE) || (dwType & INFO_ADD_PRICE_SELL)
        ...
        || (dwType & INFO_ADD_PRICE_AVATARSTORE_SELLTAB) || (dwType & INFO_ADD_PRICE_REPAIR)
...
CItem::AddItemDefaultTopTextDefenceItem(tagITEM& sItem, CInfo& Info) {
    ...
    if (sItem.GetGrade())
        // Info.SetTitle( CStr::Printf("%s (%d)",ITEM_NAME(sItem.GetTYPE(), sItem.GetItemNO() ),
        // sItem.GetGrade() )  );
        Info.AddString(CStr::Printf("%s (%d)",
                           ITEM_NAME(sItem.GetTYPE(), sItem.GetItemNO()),
                           sItem.GetGrade()),
    ...
    if (sItem.GetTYPE() != ITEM_TYPE_RIDE_PART) {
        ...
        if (sItem.GetGrade())
            stdTemp.append(
                CStr::Printf("%d", sItem.GetAvoidRate() + ITEMGRADE_AVOID(sItem.GetGrade())));
        else
...
void
CItem::AddItemEquipRequireCondition(tagITEM& sItem, CInfo& Info) {
    DWORD color = g_dwRED;
    char* pszBuf = NULL;
    ...
    switch (sItem.GetTYPE()) {
        ...
        case ITEM_TYPE_RIDE_PART:
            if (PAT_ITEM_NEED_ABILITY_IDX(sItem.GetItemNO())) {
                ...
                if (g_pAVATAR->Get_AbilityValue(PAT_ITEM_NEED_ABILITY_IDX(sItem.GetItemNO()))
                    >= PAT_ITEM_NEED_ABILITY_VALUE(sItem.GetItemNO()))
                    color = g_dwGREEN;
                else
                    color = g_dwRED;
...
void
CItem::AddItemEquipRequireUnion(tagITEM& sItem, CInfo& Info) {
    std::string strTemp("[");
    ...
    for (int i = 0; i < ITEM_EQUIP_REQUIRE_UNION_CNT; i++) {
        if (0 != ITEM_EQUIP_REQUIRE_UNION(sItem.GetTYPE(), sItem.GetItemNO(), i)) {
            ...
            strTemp.append(" ");
            strTemp.append(
                UNION_NAME(ITEM_EQUIP_REQUIRE_UNION(sItem.GetTYPE(), sItem.GetItemNO(), i)));

            if (ITEM_EQUIP_REQUIRE_UNION(sItem.GetTYPE(), sItem.GetItemNO(), i)
                == g_pAVATAR->GetCur_UNION()) {
                bCorrect = true;
                break;
            }
        }
...
void
CItem::AddItemEquipRequireJob(tagITEM& sItem, CInfo& Info) {
...
void
CItem::AddItemSpecialJob(tagITEM& sItem, CInfo& Info) {
    char* pszBuf;
    int iJobIndex = ITEM_NEED_UNION(sItem.GetTYPE(), sItem.GetItemNO(), 0);
    if (iJobIndex) {
        pszBuf = CStr::Printf("[%s %s]",
            CStringManager::GetSingleton().GetJobName(iJobIndex),
            GetAddDataValueString(sItem, 0));
...
void
CItem::AddItemSpecialUnion(tagITEM& sItem, CInfo& Info) {
    char* pszBuf;
...
void
CItem::AddItemDefence(tagITEM& sItem, CInfo& Info) {
    short nItemType = sItem.GetTYPE();
    short nItemIdx = sItem.GetItemNO();
...
void
CItem::AddItemUseNeedData(tagITEM& sItem, CInfo& Info) {
    short nItemIdx = sItem.GetItemNO();
...
void
CItem::AddItemMoveSpeed(tagITEM& sItem, CInfo& Info) {
    short nItemType = sItem.GetTYPE();
    short nItemIdx = sItem.GetItemNO();
    ...
    switch (nItemType) {
        ...
        case ITEM_TYPE_BOOTS:
            pszBuf = CStr::Printf("[%s %d]",
                STR_MOVE_SPEED,
                BOOTS_MOVE_SPEED(nItemIdx) - iDefaultMoveSpeed);
            Info.AddString(pszBuf);
            break;
            ...
            for (int j = 0; j < 2; j++) {
                ...
                if (nAttribute != 0) {
                    if (nAttribute == AT_SAVE_MP)
                        pszBuf = CStr::Printf("[%s %d%%]",
                            CStringManager::GetSingleton().GetAbility(nAttribute),
                            GetOption(sItem, nOption, j));
                    else
                        pszBuf = CStr::Printf("[%s %d]",
                            CStringManager::GetSingleton().GetAbility(nAttribute),
                            GetOption(sItem, nOption, j));
...
void
CItem::AddItemPrice(tagITEM& sItem, DWORD dwDlgType, DWORD dwType, CInfo& Info) {
    char* pszBuf = NULL;
    int iPrice = 0;
    ...
    if (dwType & INFO_ADD_PRICE_STORAGE_FEE) {
        ...
        if (sItem.IsEnableKEEPING()) ///보관료 표시
        {
            ...
            int iStorageFee =
                CCal::Get_StorageFEE(ITEM_BASE_PRICE(sItem.GetTYPE(), sItem.GetItemNO()),
                    ITEM_PRICE_RATE(sItem.GetTYPE(), sItem.GetItemNO()),
                    iCount);
        ...
        if (dwDlgType == DLG_TYPE_ITEM) {
            ...
            if (sItem.IsEnableSELL()) {
                iPrice =
                    g_pTerrain->m_Economy.Get_ItemSellPRICE(sItem, g_pAVATAR->GetSellSkillVALUE());
        ...
        if (CStore::GetInstance().IsUnionStore()) {
            ...
            iPrice = g_pTerrain->m_Economy.Get_ItemBuyPRICE(sItem.GetTYPE(),
                sItem.GetItemNO(),
                g_pAVATAR->GetBuySkillVALUE());
            CGameUtil::ConvertMoney2String(iPrice, money_buffer, money_buffer_size);
        ...
        if ((sItem.GetTYPE() == ITEM_TYPE_RIDE_PART || sItem.IsEquipITEM())
            && sItem.GetLife() < 1000 && sItem.GetDurability() > 0) {
            __int64 i64BasePrice = ITEM_BASE_PRICE(sItem.GetTYPE(), sItem.GetItemNO());
            iRepairPrice = (i64BasePrice + 1000) * (sItem.GetDurability() + 10)
                * (1100 - sItem.GetLife()) / 400000;
        }
...
void
CItem::AddItemLifeDuration(tagITEM& sItem, CInfo& Info) {
...
short
CItem::GetAttackSpeedType(short nSpeed) // 어텍 스피드 타입( 매우빠름, 빠름... )
{
    if (nSpeed <= 9)
...
int
CItem::GetOption(tagITEM& sItem, short nOption, short nCol) {
    if (sItem.GetLife() < 1)
        return 0;
...
const char*
CItem::GetAddDataValueString(tagITEM& sItem, int iColumn) {
    ...
    return CStr::Printf(pszFormat, CStringManager::GetSingleton().GetAbility(iAddType), iAddValue);
}

int
CItem::GetDefence(tagITEM& sItem) {
    if (sItem.GetLife() < 1)
        return 0;
...
int
CItem::GetAttackPower(tagITEM& sItem) {
    if (sItem.GetLife() < 1)
        return 0;
...
void
CItem::AddItemPatAddAbility(tagITEM& sItem, CInfo& Info) {
    for (int i = 0; i < PAT_ITEM_ADD_ABILITY_CNT; ++i) {
        if (PAT_ITEM_ADD_ABILITY_IDX(sItem.GetItemNO(), i)) {
            Info.AddString(CStr::Printf("[%s %d]",
                               CStringManager::GetSingleton().GetAbility(
                                   PAT_ITEM_ADD_ABILITY_IDX(sItem.GetItemNO(), i)),
...
#ifdef FRAROSE
int
CItem::GetItemRareType(int type, int no) {
    switch (type) {
        case ITEM_TYPE_WEAPON:
        case ITEM_TYPE_ARMOR:
...
CIcon*
CItemFragment::CreateItemIcon() {
    CIcon* pIcon = new CIconItem(this);
    return pIcon;
}

int
CItemFragment::GetQuantity() {
    return m_iQuantity;
}

void
CItemFragment::SetQuantity(int iQuantity) {
    m_iQuantity = iQuantity;
}

CItem*
CItemFragment::GetInnerCItem() {
    assert(m_pItem);
    return m_pItem;
}

void
CItemFragment::GetToolTip(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
    assert(m_pItem);
...
void
CItemFragment::SetUnitPrice(int iUnitPrice) {
    m_pItem->SetUnitPrice(iUnitPrice);
}

int
CItemFragment::GetUnitPrice() {
