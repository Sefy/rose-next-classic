...
GetClanNameColor(int iClanLevel) {
    switch (iClanLevel) {
        case 1:
            return g_dwBlueName;
        case 2:
            return g_dwGreenName;
        case 3:
            return g_dwYellowName;
        case 4:
            return g_dwOrangeName;
        case 5:
            return g_dwRedName;
        case 6:
            return g_dwPinkName;
        case 7:
            return g_dwVioletName;
        default:
            break;
    }
    return g_dwGrayName;
...
void
CNameBox::Draw(float x, float y, float z) {
    // Transform
    D3DXMATRIX mat;
    ...
    /// guild name
    if (m_strGuildName != "")
        ::drawFont(g_GameDATA.m_hFONT[FONT_SMALL_BOLD],
            true,
            &rt,
            g_dwYELLOW,
            DT_LEFT,
            (char*)m_strGuildName.c_str());
...
CNameBox::Draw(CObjCHAR* pCharOBJ, float x, float y, float z) {
    ...
    bool bTargetObject =
        (g_UserInputSystem.GetCurrentTarget() == pCharOBJ->Get_INDEX()) ? true : false;
...
CNameBox::DrawNpcName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted) {
    ...
    switch (pNpc->m_nQuestSignal) {
        case 1:
            ...
            QuestEmoticonGid =
                CResourceMgr::GetInstance()->GetImageNID(0, "QUEST_EMOTICON_POSSIBLE");
            break;
        case 2:
            bDrawQuestEmoticon = true;
        ...
        case 3:
            bDrawQuestEmoticon = true;
            QuestEmoticonGid =
                CResourceMgr::GetInstance()->GetImageNID(0, "QUEST_EMOTICON_COMPLETE");
            break;
        default:
...
CNameBox::DrawMobName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted) {
    ...
    if (bTargeted) {
        g_DrawImpl.Draw(x - iWidthBackImage / 2,
        ...
        g_DrawImpl.Draw(x - iWidthGuage / 2,
            y - NAMEBOX_HEIGHT / 2 + 4,
            z,
            iWidth,
            IMAGE_RES_UI,
        ...
        if (pszMobName) {
            ...
            RECT rc = {iWidthGuage / 2 - size.cx / 2 - 5,
                -18,
                iWidthGuage / 2 + size.cx / 2 + 5,
                0};
        ...
        SIZE size =
            getFontTextExtent(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], pCharOBJ->Get_NAME());
...
CNameBox::DrawAvatarName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted) {
    ...
    if (g_ClientStorage.IsShowPartyMemberHpGuage()
        && CParty::GetInstance().IsPartyMember(
            g_pObjMGR->Get_ServerObjectIndex(pCharOBJ->Get_INDEX()))) {
        ...
        g_DrawImpl.Draw(fGuageDrawX,
            fGuageDrawY,
            z,
            IMAGE_RES_UI,
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI00_GUAGE_BG_AVATAR"),
    ...
    if (pCharOBJ->GetClanID()) {
        ...
        drawFont(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE],
            ...
            0,
            0,
            GetClanNameColor(pCharOBJ->GetClanLevel()),
            pCharOBJ->GetClanName());
    }
}
...
CNameBox::DrawMyName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted) {
    ...
    if (g_pAVATAR->GetClanID()) {
        ...
        drawFont(g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE],
            ...
            0,
            0,
            GetClanNameColor(g_pAVATAR->GetClanLevel()),
            g_pAVATAR->GetClanName());
    }

    if (bTargeted) {
        RECT rcDrawTargetMark = {fGuageDrawX,
            fGuageDrawY,
            fGuageDrawX + iWidthGuage,
            fGuageDrawY + iHeightGuage};
        DrawTargetMark(g_pAVATAR, rcDrawTargetMark, z);
    }
}

void
CNameBox::DrawTargetMark(CObjCHAR* pChar, RECT& rcDrawName, float z) {
    assert(pChar);
    if (pChar == NULL)
        return;

    int iTargetMarkImageID =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI00_TARGETMARK");
    ...
    float fTargetMarkDrawY =
        (float)(rcDrawName.top + (rcDrawName.bottom - rcDrawName.top) / 2 - iTargetMarkHeight / 2);
