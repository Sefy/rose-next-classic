...
CUpgradeDlgStateResult::CUpgradeDlgStateResult(CUpgradeDlg* pParent) {
    ...
    m_iRedGuageImageID =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "ID_GUAGE_MAKE_RED");
    m_iGreenGuageImageID =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "ID_GUAGE_MAKE_GREEN");
...
void
CUpgradeDlgStateResult::Update(POINT ptMouse) {
    if (m_bWaitUserinput)
        return;
    ...
    if (m_pResultGuage->Update(ptMouse, m_dwPrevTime, dwCurrTime) == CGuage::UPDATE_END) {
        ...
        switch (btResult) {
            case CRAFT_UPGRADE_SUCCESS: {
                CTCmdChangeStateUpgradeDlg* pCmd =
                    new CTCmdChangeStateUpgradeDlg(CUpgradeDlg::STATE_NORMAL);
                g_itMGR.OpenMsgBox(STR_CRAFT_UPGRADE_SUCCESS,
                    CMsgBox::BT_OK,
                    true,
                    0,
                    pCmd); ///해당 아이템을 보여주자.
...
void
CUpgradeDlgStateResult::Draw() {
    m_pResultGuage->Draw();
}
void
CUpgradeDlgStateResult::MoveWindow(POINT ptPosition) {
