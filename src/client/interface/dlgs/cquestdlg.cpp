...
const POINT QUEST_SLOT_POSITION[QUEST_SLOT_MAX] = {{10, 347 - 171},
    {51, 347 - 171},
    {92, 347 - 171},
    {133, 347 - 171},
    {174, 347 - 171},
...
unsigned int
CQuestDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
bool
CQuestDlg::On_LButtonUP(unsigned ID) {
    switch (ID) {
...
bool
CQuestDlg::On_LButtonDN(unsigned ID) {
    switch (ID) {
        case IID_ZLIST_ITEM: {
...
void
CQuestDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
    ...
    for (int i = 0; i < QUEST_SLOT_MAX; ++i) {
        ...
        if (PtInRect(&rcSlot, ptMouse)) {
            ...
            if (pItem && !pItem->IsEmpty()) {
                ...
                Spliter.Clear();
                Spliter.DoSplit((char*)ITEM_DESC(pItem->GetTYPE(), pItem->GetItemNO()),
                    30,
...
void
CQuestDlg::UpdateQuestDescription() {
    ...
    for (int i = 0; i < QUEST_SLOT_MAX; ++i) {
        ...
        if (pItem && !pItem->IsEmpty()) {
            ...
            if (pItem->IsEnableDupCNT()) {
                int iImageIndex =
                    CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "ID_BLACK_PANEL");
                char szCount[32] = {0};
...
CWinCtrl*
CQuestDlg::FindChildInPane(unsigned uiPaneID, unsigned uiChildID) {
...
//*-------------------------------------------------------------------------------------*/
void
CQuestDlg::ActiveListBoxMoveBegin() {
