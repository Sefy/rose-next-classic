...
CDigitEffect::Init() {
    ...
    m_hMotion = ::loadMotion("DigitEffectMotion",
    ...
    for (i = 0; i < MAX_DIGIT_EFFECT; i++) {
        hMat = ::loadNullColormapMaterial(CStr::Printf("DigitEffect_%d", i),
            g_GameDATA.m_hShader_nolit,
            DIGIT_WIDTH * DIGIT_COUNT,
            DIGIT_HEIGHT);
        // hMat = ::loadColormapMaterial( CStr::Printf ("DigitEffect_%d", i ),
        // g_GameDATA.m_hShader_nolit, "3DData\\Effect\\Special\\DigitNumber01.tga" );
        if (hMat == NULL) {
            ClearAll();
            return false;
        }
        ...
        hMesh =
            ::loadMesh(CStr::Printf("DigitMesh_%d", i), "3DData\\Effect\\Special\\DigitWork01.zms");
        if (hMesh == NULL) {
            ClearAll();
            return false;
        }

        hAnimatable = ::loadAnimatable(CStr::Printf("DigitNode_%d", i),
            hMesh,
            m_hMotion,
            hMat,
            g_GameDATA.m_hShader_nolit);
        ...
        ::setMaterialBlendTypeCustom(hMat, D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, D3DBLENDOP_ADD);
...
CDigitEffect::CreateDigitEffect(int iPoint, float x, float y, float z, bool bIsUSER) {
    ...
    if (iPoint == 0) {
        ...
        for (int i = 0; i < iDigitCount; i++) {
            SetRect(&SourceRect,
                (DIGIT_WIDTH * iDigit[DIGIT_COUNT - iDigitCount + i]),
                0,
                (DIGIT_WIDTH * iDigit[DIGIT_COUNT - iDigitCount + i]) + DIGIT_WIDTH,
                DIGIT_HEIGHT);
