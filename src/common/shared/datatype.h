/*
    ** 모델 데이타 text 작성법.
    ...
    obj 1
        ...
        isskin   	0					-- skinning		on:1, off:0		*주의 bonenumber 같이
   사용안된다 ! alpha 		0					-- alpha		on:1, off:0 twoside 	0
   -- twoside		on:1, off:0 alphatest	1                   -- 알파테스트 여부. on:1, off:0.
   2003/10/10 추가됨. alpharef	128                 -- 알파테스트시 비교값. 0-255. 2003/10/10
   추가됨. ztest		1                   -- Z테스트.  on:1, off:0.                     2003/10/10
   추가됨. zwrite		1                   -- Z값 쓰기. on:1, off:0.                     2003/10/10
   추가됨. blendtype	0                   -- 블렌딩타입. 기본:0, 밝게:1.                2003/10/10
   추가됨. specular	0                   -- 스페큘러 여부. on:1, off:0.				  2003/10/10
   추가됨. alphavalue 1.0				-- 재질 기본 전체 알파. 기본값 1.0		2004/07/23 추가됨.
        glowtype 0					-- 재질 글로우 타입 기본값 0					2004/07/25
   추가됨. glowcolor 0.5 0.5 0.5    -- 글로우 컬러. 기본 1, 1, 1. 범위 0에서 1사이. 2004/07/22
   추가됨. pos	0.0 0.0 0.0					-- export에서 기록되는 정보 rot -0.xxx 0.xxx -0.xxx
   -0.xxx	-- export에서 기록되는 정보 scale 1.0 1.0 1.0				-- export에서 기록되는 정보
        linkdummy	?					-- 장착 아이템의 경우 위치할 더미 번호.
        bonenumber	?					-- 위치할 본의 번호				*주의 isskin과 같이
   사용안된다 ! parent		0
        collision	4					-- 충돌 체크, 필드 아이템, 건물 나무등은 4입력  풀,
   나무잎등은 필요 없음.

        mob_stop	3DData\???.ZMO		-- 몬스터 모델중 메쉬 에니메이션이되는 경우 각 행동에 맞는
   모션 파일을 기록 mob_move	3DData\???.ZMO		-- 별도의 모션 파일이 지정이 필요 없을 경우는
   없는 추가하지 않는다. mob_attack	3DData\???.ZMO      -- 모션이 지정되지 않으면 정지 모션이
   사용된다. mob_die		3DData\???.ZMO mob_hit		3DData\???.ZMO mob_etc		3DData\???.ZMO

        avt_walk	3DData\???.ZMO		-- 아바타 모델중 메쉬 에니메이션이 되는것은 각 동작에 맞는
   모션 파일을 기록. avt_attack  3DData\???.ZMO		-- 몬스터 모션 설정과 동일. avt_stop1
...
이도류    (252)
쌍총     (253)
방패      (261)	* 한손 무기를 장착했을 때, 다른 손에 보조 장비를 장착할 수 있다. * 보조 장비는
아바타의 왼손에 장착된다. 보조도구  (262)	* 팬시상품, 마법보조 도구. 약품      (311)	체력회복약,
근력강화제, DHA보강제 음식      (312)	돼지바베큐, 통닭, 주먹밥 마법문서  (313)	1회용 마법을
발동시키는 아이템 스킬교본  (314)	새로운 스킬을 배우게 해주는 아이템 보석      (411)	다이아몬드,
루비 등.. 예술품    (412)	도자기, 그림, 조각상 등.. 금속      (421)	철, 구리, 은 등.. 마금속
(422)	마법의힘이 결합된 특수 금속 석재      (423)	대리석, 기와, 벽돌 등.. 목재      (424)	판자,
각목 등.. 가죽      (425)	소가죽, 양가죽 등.. 옷감      (426)	무명천, 면, 비단, 모직 등.. 연금재료
(427)	마금속, 보석가공등에 사용 화학품    (428)	약품, 음식 등을 만들 때 사용. 글리세린, 소다,
마그네슘, 화약, 마법가루 공산품    (429)	열쇠, 인형, 종이, 고무, 유리병, 나무상자, 마법잉크(색깔
구분), 빈 마법책 자연물    (430)	철광석, 석탄, 고령토, 샘물,석유,화석 등.. 나무토막, 버섯,
잎사귀, 개미꼬리,달팽이알, 가재껍질, 생수 등 퀘스트 물품(441)	 왕의옥쇠, 마스터키, 감옥열쇠 인증서
(442)	 보안관 신분증, 국왕 초대장
*/
...
enum PET_TYPE {
    ...
    PET_TYPE_MOUNT_MIN = 41,
    PET_TYPE_MOUNT_MAX = 50,
    PET_TYPE_MAX,
};
...
enum t_AbilityINDEX {
    ...
    AT_PSV_ATK_POW_1HAND_WPN =
        42, // 공격력 : 한손	<< AT_PSV_ATK_POW_SWORD 검 에서 한손으로 변경 2004. 2. 26 by icarus
...
#define FLAG_ING_BAD                                                                           \
    (FLAG_ING_POISONED | FLAG_ING_DEC_MOV_SPEED | FLAG_ING_DEC_ATK_SPEED | FLAG_ING_DEC_APOWER \
        | FLAG_ING_DEC_DPOWER | FLAG_ING_DEC_RES | FLAG_ING_DEC_HIT | FLAG_ING_DEC_CRITICAL    \
        | FLAG_ING_DEC_AVOID | FLAG_ING_DUMB | FLAG_ING_SLEEP | FLAG_ING_FAINTING)

#define FLAG_ING_GOOD                                                                     \
    (FLAG_ING_MAX_HP | FLAG_ING_MAX_MP | FLAG_ING_INC_MOV_SPEED | FLAG_ING_INC_ATK_SPEED  \
        | FLAG_ING_INC_APOWER | FLAG_ING_INC_DPOWER | FLAG_ING_INC_RES | FLAG_ING_INC_HIT \
...
const DWORD c_dwIngFLAG[] = {
    ...
    FLAG_ING_DUMB, //	ING_DUMB,				// 유지 : 벙어리 : skill stb 5번 컬럼이 7~14인 스킬
                   //사용 못함.
    FLAG_ING_SLEEP, //	ING_SLEEP,				// 유지 : 수면 : 명령 불능 상태로.. 공격 받으면 풀림
    FLAG_ING_FAINTING, //	ING_FAINTING,			// 유지 : 기절 : 수면과 같으나 공격 받아도
                       //안풀림
...
#define ITEM_OBJ_LIVE_TIME \
    ((60 * 1000) + ITEM_OBJ_FREE_TIME) /// 아이템 처름 생성시 사라지기 까지 시간.
#define ITEM_OBJ_DROP_TIME \
    ((30 * (60 * 1000)) + ITEM_OBJ_FREE_TIME) /// 인벤토리가 모자라 바닦에 떨어진 아이템
