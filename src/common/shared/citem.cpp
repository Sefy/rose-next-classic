...
tagBaseITEM::GetShotTYPE() {
    ...
    if (ITEM_TYPE_WEAPON == m_cType) {
        switch (WEAPON_TYPE(m_nItemNo)) {
            ...
            case WEAPON_ITEM_USE_ARROW2:
                return SHOT_TYPE_ARROW;
            case WEAPON_ITEM_USE_BULLET:
            case WEAPON_ITEM_USE_BULLET2:
                return SHOT_TYPE_BULLET;
            case WEAPON_ITEM_USE_THROW:
                return SHOT_TYPE_THROW;
        }
    }
...
const t_EquipINDEX s_ItemType2EquipPos[] = {
    ...
    EQUIP_IDX_NECKLACE, // ITEM_TYPE_JEWEL,				// 7	LIST_JEWEL.stb		장신구 : 목걸이
                        // 반지
    ...
    if (m_cType >= ITEM_TYPE_FACE_ITEM && m_cType <= ITEM_TYPE_SUBWPN) {
        ...
        if (eIDX == EQUIP_IDX_NECKLACE) {
            switch (ITEM_TYPE(m_cType, m_nItemNo)) {
                case JEWEL_ITEM_RING:
                    return EQUIP_IDX_RING;
                case JEWEL_ITEM_NECKLACE:
                    return EQUIP_IDX_NECKLACE;
                case JEWEL_ITEM_EARRING:
                    return EQUIP_IDX_EARRING;
            }
...
#ifndef __SERVER
char*
tagBaseITEM::GettingMESSAGE(int iInventoryListNo) {
...
#ifndef __SERVER
char*
tagBaseITEM::GettingQuestMESSAGE() {
    if (m_cType == ITEM_TYPE_MONEY)
...
#ifndef __SERVER
char*
tagBaseITEM::SubtractQuestMESSAGE() {
    if (m_cType == ITEM_TYPE_MONEY)
...
#ifndef __SERVER
t_eSHOT
tagBaseITEM::GetNaturalBulletType(int iItemNo) {
...
#ifndef __SERVER
bool
tagBaseITEM::IsEqual(int iType, int iItemNo) {
