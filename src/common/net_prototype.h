...
#define WSV_CHAR_CHANGE \
    0x071c // 이 패킷 받으면 CLI_CHAR_LIST을 서버에 요청 :: 월드 서버에서 전송되어짐
...
#define GSV_SCREEN_SHOT_TIME \
    0x07eb //스크린샷 찍을때필요한 서버 시간전송.. .(허재영 추가 2005.10.18)
...
3	:	// 패스워드 에러 (login result)	계정 또는 패스워드 에러 또는 포인트 충전하지 않은 상태,
Joypark 회원 중심 참고. 4	:	// ObjnetObiect 구축 실패, Login Server등록 불가능 (Connection State
Result) 5	:	// Login Server연결 중단 (Connection State Result) 6	:	// Login Server연결 실패
(Connection State Result) 7	:	// 없는 계정 (login result)	계정 또는 패스워드 에러 또는 포인트를
충전하지 않은 상태,  Joypark 회원 중심 참고. 8	:	// 포인트 부족 (login result)	포인트 부족 9
:	// 시간 부족 (login result)	무료 체험기간 만기 10	:	// 없는 과금제도 (login result)	과금방식
문제 11	:	// 블록 (login result)	블록 중 12	:	// 블록３일 (login result)	블록 중 13	: //
블록７일 (login result)	블록 중 14	:	// 블록 10 일 (login result)	블록 중 15	:	// 로그 아웃
상태 (logout result) 16	:	// 불확실한 에러 (데이터 베이스 에러 login, relogin, logout result)
불확실한 에러 17	:	// Ip중복 (홈 유저 대상 login, relogin, result) 18	:	// 게임방 유효한
게임방 ip리스트 (사용 : 게임방 login, relogin, result)
20	:	// 없는 계정 (relogin result)	계정 또는 패스워드 에러 도는 포인트를 충전하지 않은 상태,
Joypark 회원 중심 참고. 21	:	// 중복 로그인 (relogin result)	이미 온라인 상황인 계정 23	: //
포인트 부족 (relogin result)	포인트 부족 24	:	// 시간 부족 (relogin result)	무료 체험기간
만기 25	:	// 부정확한 과금방식 (relogin result)	과금방식 문제 26	:	// 블록 (relogin result)
블록 중 27	:	// 블록 ３일 (relogin result)	블록 중 28	:	// 블록 ７일 (relogin result) 블록
중 29	:	// 블록 10 일 (relogin result)	블록 중 30	:	// 같은 ip  n초내 중복 로그인 (login
result)	마우스를 잠깐 쉬게 하고, 몇 초 후 다시 접속. 31	:	// Game Server등록 성공 (Connection
State result) 32	:	// Game Server등록 ID중복(Connection State result) 33	:	// Game Server미
등록(login, relogin result)	서버 접속 불가능, 잠시후 다시 연결 시도. 34	:	// 유저 매일 접속 가능한
시간 초과.(login, relogin result)	접속시간 초과.
...
#define RESULT_JOIN_WORLD_TIME_OUT				0x02    // 시간 초과로 계정 없어져 다시 로그인
해야한다. #define RESULT_JOIN_WORLD_INVALID_PASSWORD		0x03    // 비번 오류 #define
RESULT_JOIN_WORLD_ALREADY_LOGGEDIN		0x04    // 이미 로그인 중이다
*/
...
struct tag_ADD_CHAR: public t_PACKETHEADER {
    ...
    WORD m_wCommand;
    WORD m_wTargetOBJ;
    BYTE m_btMoveMODE; // 0:걷기, 1:뛰기, 2:내 승용아이템에 타고있다, 3: m_wTargetOBJ의 승용아이템에
                       // 타고있다.
    int m_iHP;
#ifdef __APPLY_04_10_15_TEAMNO
...
// tag_DROPITEM은 몹이 죽으면서 떨구는거...아이템 생성 시간은 고정이름 클라이어트가
// ITEM_OBJ_LIVE_TIME 으로 설정.
struct tag_DROPITEM {
    tPOINTF m_PosCUR;
    tagBaseITEM m_ITEM;
...
struct cli_EQUIP_ITEM: public t_PACKETHEADER {
    short m_nEquipInvIDX; // 장착창 인벤토리 번호 :: m_nEquipInvIDX >= 1 && m_nEquipInvIDX <
                          // MAX_EQUIP_IDX
...
struct gsv_EQUIP_ITEM: public t_PACKETHEADER {
    ...
    short m_nRunSPEED[0]; // 패시브 스킬에의해 보정된 값까지 : 지속에 의해 보정된값 제외 :: 팻 미
                          // 탑승 안했을때..
};
...
struct gsv_ASSEMBLE_RIDE_ITEM: public t_PACKETHEADER {
    ...
    short m_nRunSPEED[0]; // 패시브 스킬에의해 보정된 값까지 : 지속에 의해 보정된값 제외 :: 팻 탑승
                          // 했을때...
};
...
#define PARTY_REPLY_CHANGE_OWNERnDISCONN \
    0x09 // PARTY_REPLY_CHANGE_OWNER와 같다...단 기존 파짱을 disconnect된 상태로 변경.
...
#define GCMD_LEADER \
    0x009 //길드위임, /gleader <플레이어> - 다른 플레이어에게 길드장 위임하기 (길드장 전용)
...
#define GCMD_MEMBER_JOBnLEV \
    0x00f //자신의 직업/레벨을 다른 멤버에게 통보( 로그인후/레벨업후/전직후에 전송 )
...
#define RESULT_QUEST_REPLY_TRIGGER_SUCCESS \
    0x05 // 이 패킷을 받으면 클라이언트에서도 보상을 적용한다.
...
//(2004년06월01일)
//#define	BILLING_MSG_FEE_TYPE2				0x02	//         귀하는 정량요금제 사용자 입니다.
//(18시간20분20초)
...
#define BILLING_MSG_PAY_GQ \
    0x06 // GQ	게임방에서 로그인을 시도하였으나 게임방의 과금기간(시간)이 만료되어 개인정량으로
         // 로그인 되었음을 나타낸다.
#define BILLING_MSG_PAY_IQ \
    0x07 // IQ	게임방에서 로그인을 시도하였으나 이미 계약된 IP 숫자를 모두 사용하고 있어
         // 개인정량으로 로그인 되었음을 나타낸다.
...
#define BILLING_MSG_JPN_NEED_CHARGE \
    0x41 // 일본 과금 충전 필요!!! gsv_BILLING_MESSAGE2 패킷구조로 전송되고 m_dwPayFlag에 필요한
         // 과금 플레그 셋팅되어 있음

//#define	BILLING_MSG_TIME_ALERT				0x31	// 남은 시간 통보
//#define	BILLING_MSG_TIME_EXPIRED			0x32	// <종료> 결제 시간이 다되어 게임을
//종료합니다.
...
// 한국 과금 결제 비트~
#define PAY_FLAG_KR_FU \
    (0x01 << BILLING_MSG_PAY_FU) // FU	로그인 된 사용자는 무료 아이디 사용자임을 나타낸다.
#define PAY_FLAG_KR_FA \
    (0x01 << BILLING_MSG_PAY_FA) // FA	로그인 된 사용자는 개인정액 사용자임을 나타낸다.
#define PAY_FLAG_KR_FAP \
    (0x01 << BILLING_MSG_PAY_FAP) // FAP	로그인 된 사용자는 개인정액 사용자(플레티넘)임을
                                  // 나타낸다.
#define PAY_FLAG_KR_FQ \
    (0x01 << BILLING_MSG_PAY_FQ) // FQ	로그인 된 사용자는 개인정량 사용자임을 나타낸다.
#define PAY_FLAG_KR_GU \
    (0x01 << BILLING_MSG_PAY_GU) // GU	로그인 된 사용자는 게임방 사용자임을 나타낸다.
#define PAY_FLAG_KR_GQ \
    (0x01 << BILLING_MSG_PAY_GQ) // GQ	게임방에서 로그인을 시도하였으나 게임방의 과금기간(시간)이
                                 // 만료되어 개인정량으로 로그인 되었음을 나타낸다.
#define PAY_FLAG_KR_IQ \
    (0x01 << BILLING_MSG_PAY_IQ) // IQ	게임방에서 로그인을 시도하였으나 이미 계약된 IP 숫자를 모두
                                 // 사용하고 있어 개인정량으로 로그인 되었음을 나타낸다.

// #define	PAY_FLAG_KR_PLATINUM		( FLAG_PAY_FAP|FLAG_PAY_GU )

// 일본 과금 결제 비트~
//#define	FLAG_JPAY_CHAT			(0x01 << 0)						// 0	Free Play 	캐릭작성,
//채팅
...
// 한국 기본 과금
#define PLAY_FLAG_KOREA_DEFAULT                                                       \
    (PLAY_FLAG_BATTLE | PLAY_FLAG_COMMUNITY | PLAY_FLAG_TRADE | PLAY_FLAG_STOCK_SPACE \
        | PLAY_FLAG_STARSHIP_PASS | PLAY_FLAG_DUNGEON_ADV)
...
#define EXT_BILLING_MSG_TYPE_SET_DATE \
    0x1010 // 만료 일자 설정	m_dwPayType = m_dwPlayingFlag = 0	szMsg =
           // 타입\t200309101025\t타입\t200309101025...
#define EXT_BILLING_MSG_TYPE_SET_TIME \
    0x1011 // 만료 시간 설정	m_dwPayType = m_dwPlayingFlag = 0	szMsg =
           // 타입\t남은분\t타입\t남은분...
#define EXT_BILLING_MSG_TYPE_TIME_EXPIRED \
    0x1012 // PayType=0이고 m_dwPlayingFlag[0]에 만료된 과금 플레그있음
#define EXT_BILLING_MSG_TYPE_TIME_ALERT \
    0x1013 // PayType=0이고 m_dwPlayingFlag[0]에 시간경고되는 과금 플레그있음
