...
bool
CTEditBox::Create(int iScrX, int iScrY, int iWidth, int iHeight, int iCharWidth) {
    m_iMaxCharCnt = 126;
...
void
CTEditBox::SetMultiline(bool bMultiline) {
    ...
    switch (wParam) {
        case VK_LEFT: {
            if (m_iPromptPos) {
                ...
                char* pPrevPos = CharPrevExA(CTControlMgr::GetInstance()->GetCodePage(),
                    m_pszBuffer,
                    pCurrPos,
                    0);
                if (pCurrPos != pPrevPos) {
                    ...
                    if (m_iPromptPos < m_iViewPos) {
                        ...
                        const char* pPrevViewPos =
                            CharPrevExA(CTControlMgr::GetInstance()->GetCodePage(),
                                m_pszBuffer,
                                pCurrViewPos,
                                0);
            ...
            if (pCurrPos != pNextPos) {
                ...
                if (m_iPromptPos > m_iViewPos) {
                    ...
                    if (size.cx > m_iWidth) {
                        ...
                        char* pNextViewPos = CharNextExA(CTControlMgr::GetInstance()->GetCodePage(),
                            pCurrViewPos,
                            0);
...
void
CTEditBox::SetFocus(bool bFocus) {
    ...
    if (bFocus) {
        ...
        if (s_pFocusEdit && s_pFocusEdit == this) {
            s_pFocusEdit = NULL;
            HIMC old_HIMC =
                ImmAssociateContext(CTControlMgr::GetInstance()->GetWindowHandle(), NULL);
...
/// @warning WM_KEYUP 처리에 주의할것, 현재 변환중 Key Up에 대한 메세지가 오고 있다.( 여기서 다른
/// 처리시 일본어/중국어 변환시에 sideeffect발생가능성이 높다 )
        ...
        case WM_KEYDOWN: /// Enter와 Esc처리하자.
        {
            if (IsFocus()) {
                if ((wParam == VK_ESCAPE || wParam == VK_RETURN)
                    && CTControlMgr::GetInstance()->GetKeyboardInputType()
...
CTEditBox::Draw() {
    ...
    if (IsFocus() && !IsTypeHideCursor()) {
        ...
        m_pFontMgr
            ->Draw(m_iFont, true, &rcDraw, m_dwTextColor, 0, /*TIme.GetStrPrompt().c_str()*/ "_");
        ...
        if (!compClauseInfo.empty()) {
            ...
            for (iterClauseInfo = compClauseInfo.begin(); iterClauseInfo != compClauseInfo.end();
                 ++iterClauseInfo) {
        ...
        if( int iCount = count( compAttr.begin(), compAttr.end(), 1 ) )
        {
            ...
            m_pFontMgr->Draw( m_iFont, true, rcDraw.left, rcDraw.top, m_dwTextColor,
        strunderline.c_str() );
        }
        */
...
void
CTEditBox::SetText(const char* szTxt) {
    if (szTxt == NULL)
        return;
...
bool
CTEditBox::chk_number(const char* szText) {
    if (szText == NULL)
        return false;
...
void
CTEditBox::delete_one_char(int iType) {
    /// 0 : delete key
    /// 1 : backspace
    switch (iType) {
        case 0: {
        ...
        case 1: {
            char* pCurrPos = &m_pszBuffer[m_iPromptPos];
            char* pPrevPos =
                CharPrevExA(CTControlMgr::GetInstance()->GetCodePage(), m_pszBuffer, pCurrPos, 0);
            if (pCurrPos != pPrevPos) {
                if (m_iViewPos) {
                    const char* pCurrViewPos = &m_pszBuffer[m_iViewPos];
                    const char* pPrevViewPos =
...
void
CTEditBox::AppendText(const char* szText) {
    if (szText == NULL)
        return;
...
void
CTEditBox::SetTextColor(int iColor) {
    switch (iColor) {
        case 1:
...
void
CTEditBox::SetCharHeight(int iHeight) {
...
bool
CTEditBox::OnChar(WPARAM wParam, LPARAM lParam) {
    switch (wParam & 0xff) {
        case VK_BACK: ///
        {
            char* pCurrPos = &m_pszBuffer[m_iPromptPos];
        ...
        default: {
            if (((wParam & 0xff) != 27) && ((wParam & 0xff) != 9) && (wParam != VK_RETURN)) {
                ...
                MoveMemory(&m_pszBuffer[m_iPromptPos + 1],
                    &m_pszBuffer[m_iPromptPos],
                    strlen(m_pszBuffer) - m_iPromptPos);
...
void
CTEditBox::Insert(const char* text) {
    if (text == NULL)
        return;
    ...
    if (iOrgLen <= m_iPromptPos) {
        ...
        MoveMemory(&m_pszBuffer[m_iPromptPos + iLen],
            &m_pszBuffer[m_iPromptPos],
            iOrgLen - m_iPromptPos);
